...
import { businessCardTemplates } from '@/ComponentsPageBuilder/businessCardTemplates';

const pageBuilderService = getPageBuilder();
const open = ref(false);

const templates = businessCardTemplates;
...
<template>
    <div>
        <button type="button" class="btn btn-outline" @click="open = true">Plantillas de Tarjeta</button>

        <dialog class="modal" :open="open">
            <div class="modal-box max-w-full min-h-full flex flex-col">
                <h3 class="font-semibold text-lg">Plantillas de Tarjeta de Negocio</h3>
                <p class="text-sm opacity-70 mt-1">Selecciona una plantilla para tu tarjeta de negocio digital e insértala en tu canvas.</p>
                ...
                <div class="mt-4 flex gap-6 flex-1 overflow-hidden">
                    ...
                    <div class="flex-1 flex flex-col space-y-4 overflow-hidden">
                        <div class="flex-1 rounded border border-base-300 bg-base-200 p-3 overflow-auto flex items-center justify-center">
                            <div v-if="selectedTemplate?.html_code" v-html="selectedTemplate.html_code" class="w-full h-full [&>section]:min-h-full" />
